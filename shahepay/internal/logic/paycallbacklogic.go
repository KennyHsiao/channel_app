...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequestX) (resp string, err error) {
	...
	if orderAmount, err = strconv.ParseFloat(data.Order.TotalAmount, 64); err != nil {
		return "fail", errorx.New(responsex.INVALID_AMOUNT)
	}

	orderStatus := "1"
	if data.NotifyType == "trade_completed" {
		orderStatus = "20"
	}

	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     data.Order.MerchantOrderId,
		ChannelOrderNo: data.Order.ID, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
