...
func VerifySign(reqSign string, data interface{}, screctKey string) bool {
	...
	source := fmt.Sprintf("'%s'", data) + screctKey
	sign := GetSign_SHA1(source)
	fmt.Sprintf("-------" + source)
...
func GetSign_SHA1(source string) string {
	data := []byte(source)
	source2 := fmt.Sprintf("%x", sha1.Sum(data))
	logx.Infof("sha1 %s", source2)
	//h := sha1.New()
	//result := hex.EncodeToString(h.Sum(data))
	//logx.Infof("sha1 %s", result)
	return source2
}
