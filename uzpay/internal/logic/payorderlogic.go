...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	// uid、merkey根據不同的通道而改變
	var merKey, from_bankflag string
	if req.ChannelPayType == "3" {
		uid = "55639"
		merKey = "2161963f230d7fcc7656aeeb3634b074"
		from_bankflag = "二维码"
	} else if req.ChannelPayType == "remit" {
		uid = "55640"
		merKey = "405967717d0f5371031d2a73c1eb9951"
		from_bankflag = "BANK"
	} else if req.ChannelPayType == "1" {
		uid = "55641"
	...
	//data.Set("sign", sign)
	sign := payutils.SortAndSignFromObj(data, merKey)
	data.Sign = sign
	data.Extend = uid
