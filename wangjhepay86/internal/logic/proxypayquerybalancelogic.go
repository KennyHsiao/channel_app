...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	ChannelResp, ChnErr := gozzle.Post(channel.ProxyPayQueryBalanceUrl).Header("Authorization", "api-key "+l.svcCtx.Config.ApiKey.ChannelKey).Timeout(20).Trace(span).JSON(data)
