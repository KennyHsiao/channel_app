...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	ChannelResp, ChnErr := gozzle.Post(channel.ProxyPayQueryBalanceUrl).Header("server", "true").Timeout(20).Trace(span).Form(data)
	...
	} else if balanceQueryResp.Code != 1 {
	...
	resp = &types.ProxyPayQueryInternalBalanceResponse{
		...
		ProxyPayBalance:    balanceQueryResp.Data.Money,
