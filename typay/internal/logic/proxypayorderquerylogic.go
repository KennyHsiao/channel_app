...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	ChannelResp, ChnErr := gozzle.Post(channel.ProxyPayQueryUrl).Header("server", "true").Timeout(20).Trace(span).Form(data)
	...
	} else if channelQueryResp.Code != 1 {
	...
	var orderStatus = "1"
	if channelQueryResp.Data.Tradestatus == "WITHDRAWAL_SUCCESS" {
		orderStatus = "20"
	} else if strings.Index("WITHDRAWAL_FAILURE", channelQueryResp.Data.Tradestatus) > -1 {
		orderStatus = "30"
	}
