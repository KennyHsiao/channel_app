...
import (
	...
	"github.com/copo888/channel_app/typay/internal/service"
...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	data.Set("nonce", randomID)
	data.Set("tid", req.OrderNo)
	data.Set("money", transactionAmount)
	data.Set("callback_url", notifyUrl)
	data.Set("bankcode", "000")
	data.Set("bankname", req.ReceiptCardBankName)
	data.Set("ifsc", channelBankMap.MapCode)
	data.Set("bankcardnumber", req.ReceiptAccountNumber)
	data.Set("accountname", req.ReceiptAccountName)
	data.Set("time", strconv.FormatInt(timestamp, 10))
	...
	ChannelResp, ChnErr := gozzle.Post(channel.ProxyPayUrl).Header("server", "true").Timeout(20).Trace(span).Form(data)
	...
	resp := &types.ProxyPayOrderResponse{
		ChannelOrderNo: channelResp.Data.Tradebo,
		OrderStatus:    "",
	}
