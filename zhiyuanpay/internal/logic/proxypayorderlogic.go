...
import (
	...
	"github.com/copo888/channel_app/zhiyuanpay/internal/service"
...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	// 組請求參數
	transactionAmount := utils.FloatMul(req.TransactionAmount, "100", 0)
	transactionAmountS := fmt.Sprintf("%.0f", transactionAmount) // s == "123.456000"

	data := url.Values{}
	data.Set("mchId", channel.MerId)
	data.Set("mchOrderNo", req.OrderNo)
	data.Set("amount", transactionAmountS)
	data.Set("trueName", req.ReceiptAccountName)
	data.Set("cardNo", req.ReceiptAccountNumber)
	data.Set("bankType", channelBankMap.MapCode)
