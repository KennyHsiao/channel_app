...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	// 檢查驗簽
	//if isSameSign := payutils.VerifySign(req.Mac, *req, channel.MerKey); !isSameSign {
	//	return "fail", errorx.New(responsex.INVALID_SIGN)
	//}

	var orderAmount float64
	orderAmount = utils.FloatDiv(req.TxnAmt, "100")
	//if orderAmount, err = strconv.ParseFloat(req.TxnAmt, 64); err != nil {
	...
	payCallBackBO := bo.PayCallBackBO{
		...
		ChannelOrderNo: req.TxnId,   // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
